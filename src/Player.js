...
class Player {
  ...
  get heroSummary() {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      summary += `**${i + 1}:** ${this.heroCollection.heroNameFromNumber(hero.hero_id)} (played: ${hero.games}, `;
      summary += `winrate: ${this.heroWinrate(hero.games, hero.win)}%)\n`;
    }
    return summary;
  }
  ...
  embedify() {
    let embedDescription = `**Rank:** ${this.readableRank}\n**Estimated MMR:** ${this.estimatedRank}\n`;
    embedDescription += `**Wins/Losses:** ${this.wins}/${this.losses}`;
